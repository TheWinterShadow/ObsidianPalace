...
If `curl https://YOUR_DOMAIN/health` fails with a certificate error, certbot may not have run successfully. SSH into the instance and check:
...
# Re-run certbot manually if needed
docker run --rm \
...
```bash
# Check ob sync logs inside the container
...
# If auth_token is missing, re-run ob login (Step 9)
```

### OAuth flow fails

- Verify the OAuth client redirect URI matches exactly: `https://YOUR_DOMAIN/oauth2/callback`
- Check that your email is listed as a test user in the OAuth consent screen (required while the app is in "Testing" status)
- Check server logs: `docker exec obsidian-palace supervisorctl tail -f mcp-server`
...
The e2-small instance has 2 GB RAM. If your vault is very large (>1 GB), you may need to upgrade to e2-medium (4 GB). Update the `machine_type` variable in your Terraform config:

```hcl
variable "machine_type" {
  default = "e2-medium"  # 4 GB RAM
}
```
